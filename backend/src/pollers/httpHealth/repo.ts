...
export interface HttpServiceCheck {
  id: string;
  name: string;
  url: string;
  enabled: boolean;
...
function rowToCheck(r: Row): HttpServiceCheck {
  return {
    id: r.id,
    name: r.name,
    url: r.url,
...
export function listChecks(): HttpServiceCheck[] {
  const rows = getDb()
...
export function listEnabledChecks(): HttpServiceCheck[] {
  const rows = getDb()
...
export function getCheck(id: string): HttpServiceCheck | null {
  const r = getDb()
...
export function createCheck(input: CreateCheckInput): HttpServiceCheck {
  const id = randomUUID();
  const createdAt = Date.now();
  const row: HttpServiceCheck = {
    id,
...
): HttpServiceCheck | null {
  const existing = getCheck(id);
  if (!existing) return null;
  const next: HttpServiceCheck = {
    ...existing,
    ...input,
