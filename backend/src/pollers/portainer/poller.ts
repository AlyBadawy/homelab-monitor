...
export class PortainerPoller {
  ...
  private async pollEndpoint(
    ...
    for (const { c, stats, inspect } of perContainer) {
      ...
      const uptimeSec = running && inspect ? calcUptimeSec(inspect, now) : null;
      ...
      if (running && inspect?.NetworkSettings?.Networks) {
        for (const netAttach of Object.values(
   ...
   */
  private async refreshDfIfStale(
    endpointId: number,
    now: number,
  ): Promise<void> {
...
function calcCpuPct(stats: DockerContainerStats): number | null {
  ...
  const cpuDelta =
    cpu.cpu_usage.total_usage - (pre.cpu_usage?.total_usage ?? 0);
