...
export class ProxmoxPoller {
  ...
  private async pollOnce(): Promise<void> {
        ...
        for (const s of storages) {
          ...
          try {
            ...
            for (const e of backups) {
              ...
              if (vmid != null) {
                vmidsSeen.add(vmid);
                backupsByVmid.set(vmid, (backupsByVmid.get(vmid) ?? 0) + 1);
              }
            }
    ...
    const vmTargets: TargetSummary[] = vms.map((v) => {
      ...
      const memPct =
        running && v.mem != null && v.maxmem ? (v.mem / v.maxmem) * 100 : null;
    ...
    replaceByPrefix(
      "qemu-",
      vmTargets.filter((t) => t.id.startsWith("qemu-")),
    );
    replaceByPrefix(
      "lxc-",
      vmTargets.filter((t) => t.id.startsWith("lxc-")),
    );
  }
}
