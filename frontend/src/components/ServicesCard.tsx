...
/**
 * One card for all HTTP service checks. Shows a table-style grid with live
 * state + 24h history, plus inline add and two-click delete.
 *
    ...
    <div className="card col-span-full">
      ...
      <div className="mt-3 overflow-x-auto">
        <div className="min-w-[880px]">
          {/* Column widths are repeated across header + rows via the same
              grid-template — keeping them in sync is easier than
              per-cell widths. */}
          <ServiceRowGrid header>
...
/**
 * Fixed grid-template keeps header and data rows aligned. Changing any
 * column width here updates both.
 */
  ...
  return (
    <div
      className={clsx(
        ...
        'grid-cols-[minmax(220px,1fr)_80px_150px_70px_220px_40px]',
        header &&
          'border-b border-border-strong pb-2 font-mono text-[0.6rem] uppercase tracking-[0.18em] text-text-dim',
        !header && 'text-sm',
        tone === 'danger' && 'bg-accent-rose/5',
      )}
    >
      {children}
    </div>
  ...
  return (
    <ServiceRowGrid>
      ...
      <div className="flex items-center gap-2">
        <UptimeStrip points={uptime} width={140} height={14} />
      ...
      <ServiceRowGrid>
        ...
        <div className="flex flex-col gap-1">
          <input
            ...
            className="w-full rounded border border-border bg-bg-900 px-2 py-1 font-mono text-xs text-text placeholder:text-text-dim focus:border-accent-cyan focus:outline-none"
            maxLength={64}
          />
          <input
            value={url}
            ...
            className="w-full rounded border border-border bg-bg-900 px-2 py-1 font-mono text-[0.7rem] text-text placeholder:text-text-dim focus:border-accent-cyan focus:outline-none"
            type="url"
          />
          {err && (
        ...
        {/* Cols 2-5 — empty placeholder cells so column widths stay aligned */}
        <div className="text-right font-mono text-[0.65rem] text-text-dim">—</div>
        <div />
        <div className="text-right font-mono text-[0.65rem] text-text-dim">—</div>
        <div />
        ...
        <div className="flex items-center justify-end gap-1">
          <button
            type="submit"
            disabled={submitting}
            className="rounded-md border border-accent-cyan/60 px-2 py-1 font-mono text-[0.6rem] uppercase tracking-[0.18em] text-accent-cyan transition-colors hover:bg-accent-cyan/10 disabled:opacity-50"
            title="Save (Enter)"
          >
            {submitting ? '…' : 'save'}
          </button>
...
function LatencyBadge({
  ...
  if (ms === null) return <span className="text-text-dim">—</span>;
  const tone =
    ms >= 2000
      ? 'text-accent-rose'
      : ms >= 500
        ? 'text-accent-amber'
        : 'text-text';
  return <span className={tone}>{Math.round(ms)} ms</span>;
}
